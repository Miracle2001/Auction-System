    ...
    if(mysqli_num_rows($check_email_query_run) > 0){
       ...
       // $get_name = $row['sellerid'];
        ...
        if($update_token_run) {
            ...
            header("Location: buyer-forgot-password.php");
            exit(0);
...
if(isset($_POST['password-update'])) {
    ...
    if(!empty($token)){
        ...
        $check_token = "SELECT verify_token from buyer WHERE verify_token='$token' LIMIT 1";
        ...
        if(mysqli_num_rows($check_token_run) > 0){
            ...
            if($new_password == $repeat_password){
                ...
                $update_password = "UPDATE buyer SET password='$new_password_encryption' WHERE verify_token='$token' LIMIT 1";
                ...
                if($update_password_run){
                    ...
                    $_SESSION['status'] = "Something Went Wrong";
                    header("Location: buyer-change-password.php?token=$token&email=$email_address");
                    exit(0);
                }
        ...
        header("Location: buyer-change-password.php");
        exit(0);
